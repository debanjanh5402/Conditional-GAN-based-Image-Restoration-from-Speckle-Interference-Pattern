...
def _read_img(img_path:str|Path) -> torch.Tensor:
    ...
    img = resize(img.to(torch.float32), size=[IMAGE_SIZE, IMAGE_SIZE], interpolation=T.InterpolationMode.BICUBIC) # (1, 256, 256)
    img = torch.round(img)
    img = torch.clamp(img, max=255.0, min=0.0)
    img = img.to(torch.uint8)
    return img # (0, 255), torch.uint8
...
def _save_as_image(sample_dir:str|Path, name:str, tensor:torch.Tensor) -> Path:
    ...
    io.write_png(tensor, str(file_path))
    return file_path